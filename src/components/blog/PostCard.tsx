...
const PostCard = ({ post }: any) => {
  const { title, slug, excerpt, coverImage, date, type } = post.fields;
  ...
  return (
    <div className="shadow w-full sm:w-[30%] rounded-sm bg-white">
      <Link href={`/blog/${slug}`} aria-label={title} className="block">
        ...
        <div className="md:px-6 px-4 pt-2 md:pt-3 pb-4 md:pb-6">
          <p className="md:mt-2 text-xs text-blue-400">{type}</p>

          <h1 className="font-display line-clamp-title tracking-tight mt-1 text-lg font-semibold leading-none text-gray-800 ">
            {title}
          </h1>
